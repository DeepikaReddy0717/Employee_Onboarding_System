...
def UserRegisterLogic(request):
    if request.method == 'POST':
        ...
        if not re.match("^[a-zA-Z]+$", first_name):
            messages.info(request, 'First Name should only contain alphabetic characters.')
            return render(request, 'adminapp/UserRegisterPage.html')
        if not re.match("^[a-zA-Z]+$", last_name):
            messages.info(request, 'Last Name should only contain alphabetic characters.')
            return render(request, 'adminapp/UserRegisterPage.html')
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                print(f"Error during authentication: {e}")  # Debugging error output
                messages.error(request, 'Error during login. Please try again later.')

        return render(request, 'adminapp/UserLoginPage.html')

    except Exception as e:
        print(f"Unexpected error: {e}")  # Debugging error output
...
from .forms import CourseForm, ResourceFormSet
...
def view_feedback(request):
    feedbacks = Feedback.objects.all().order_by('-created_at')  # Retrieve all feedbacks sorted by latest
    return render(request, 'adminapp/view_feedback.html', {'feedbacks': feedbacks})
